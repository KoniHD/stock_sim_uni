 ...
 * 3. Run the simulation for 100 months.
 * 4. Print the performance of each wallet.
 * 5.1 Extend the simulation for 50 months.
...
int main()
{
    ...
    std::shared_ptr<StockMarket> market = std::make_shared<StockMarket>(1.0 / 12.0, 100, "../input/stocks.json");

    std::vector<Wallet> wallets;
    wallets.emplace_back(Wallet(3000, std::make_unique<LowRiskStrategy>(), market));
    wallets.emplace_back(Wallet(10'000, std::make_unique<LowRiskStrategy>(), market));
    wallets.emplace_back(Wallet(3'000, std::make_unique<HighRiskStrategy>(), market));
    wallets.emplace_back(Wallet(10'000, std::make_unique<HighRiskStrategy>(), market));
