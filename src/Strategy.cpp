...
/**
 * @brief Assesses the risk of a stock based on its variance and initial price.
 * @param stock The stock to assess.
 * @return The risk of the stock.
 * @see StockRisk
 *
 * The risk is assessed based on the standard deviation of the stock which is computed by sqrt(variance) / initialPrice.
...
StockRisk assessStockRisk(Stock stock) {
    double standardDeviation = std::sqrt(stock.getVariance()) / stock.getInitialPrice();
